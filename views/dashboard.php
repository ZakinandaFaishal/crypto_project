...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    ...
    <div class="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
        ...
        <form action="../controllers/message_controller.php" method="POST">
            <div class="mb-4">
                <label for="receiver_id_txt" class="block text-sm font-medium text-gray-400">Agen Penerima:</label>
                <select name="receiver_id" id="receiver_id_txt" required class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500 focus:ring-blue-500">
    ...
    <div class="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
        <h2 class="text-xl font-semibold mb-4 text-purple-400">3. Sinyal Gambar</h2>
        <form action="../controllers/stego_controller.php" method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-purple-700">Kirim Sinyal Gambar</button>
        </form>
    </div>

    <div class="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
        <h2 class="text-xl font-semibold mb-4 text-orange-400">4. Sinyal Audio</h2>

        <div class="flex mb-4 border-b border-gray-700">
            <button id="btn-tab-upload" class="flex-1 py-2 px-4 text-sm font-medium text-center text-white bg-gray-700 rounded-t-lg">Upload .WAV</button>
            <button id="btn-tab-record" class="flex-1 py-2 px-4 text-sm font-medium text-center text-gray-400 hover:text-white">Rekam Suara</button>
        </div>

        <div id="form-upload-container">
            <form action="../controllers/stego_audio_controller.php" method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label for="receiver_id_audio_up" class="block text-sm font-medium text-gray-400">Agen Penerima:</label>
                    <select name="receiver_id" id="receiver_id_audio_up" required class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500 focus:ring-blue-500">
                        <option value="">-- Pilih Agen --</option>
                        <?php foreach ($all_users as $user): ?>
                            <option value="<?php echo $user['id']; ?>">
                                <?php echo htmlspecialchars($user['username']); ?>
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="cover_audio" id="cover_audio" required accept="audio/wav,audio/x-wav" class="mt-1 block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-gray-700 file:text-orange-300 hover:file:bg-gray-600">
                </div>
                <div class="mb-4">
                    <label for="stego_message_audio_up" class="block text-sm font-medium text-gray-400">Pesan Rahasia:</label>
                    <textarea name="stego_message_audio" id="stego_message_audio_up" rows="2" required class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500 focus:ring-blue-500"></textarea>
                </div>
                <button type="submit" class="w-full bg-orange-600 text-white py-2 px-4 rounded-md hover:bg-orange-700">Kirim Sinyal Audio</button>
            </form>
        </div>

        <div id="form-record-container" class="hidden">
            <form id="form-audio-record" action="../controllers/audio_record_controller.php" method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label for="receiver_id_audio_rec" class="block text-sm font-medium text-gray-400">Agen Penerima:</label>
                    <select name="receiver_id" id="receiver_id_audio_rec" required class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500 focus:ring-blue-500">
                        <option value="">-- Pilih Agen --</option>
                        <?php foreach ($all_users as $user): ?>
                            <option value="<?php echo $user['id']; ?>">
                                <?php echo htmlspecialchars($user['username']); ?>
                ...
                <div class="mb-4">
                    <label for="stego_message_audio_rec" class="block text-sm font-medium text-gray-400">Pesan Rahasia:</label>
                    <textarea name="stego_message_audio" id="stego_message_audio_rec" rows="2" required class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500 focus:ring-blue-500"></textarea>
                </div>
                ...
                <button type="submit" id="btn-send-record" class="w-full bg-cyan-600 text-white py-2 px-4 rounded-md hover:bg-cyan-700 hidden">Kirim Sinyal Audio (Rekaman)</button>
            </form>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
    ...
    <div class="md:col-span-1 bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
        <h3 class="text-lg font-semibold mb-4 border-b border-gray-700 pb-2 text-purple-400">Sinyal Gambar Diterima</h3>
        <div class="space-y-3 max-h-64 overflow-y-auto">
            <?php foreach ($images->fetchAll(\PDO::FETCH_ASSOC) as $img): ?>
                <div class="border border-gray-700 p-3 rounded bg-gray-700">
                    ...
                    <img src="/skap-pemerintah<?php echo $img['image_path']; ?>" alt="Sinyal Gambar" class="w-full h-24 object-cover rounded my-2 border border-gray-600">
    ...
    <div class="md:col-span-1 bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
        <h3 class="text-lg font-semibold mb-4 border-b border-gray-700 pb-2 text-orange-400">Sinyal Audio Diterima</h3>
        <div class="space-y-3 max-h-64 overflow-y-auto">
            <?php foreach ($audios->fetchAll(\PDO::FETCH_ASSOC) as $audio): ?>
                <div class="border border-gray-700 p-3 rounded bg-gray-700">
                    ...
                    <p class="text-xs text-gray-500"><?php echo $audio['upload_at']; ?></p>
                    <audio controls class="w-full mt-2" preload="none">
                        <source src="<?php echo $audio['audio_path']; ?>" type="audio/wav">
                    </audio>
                    <a href="read_audio.php?id=<?php echo $audio['id']; ?>" class="text-orange-400 hover:underline text-sm">Dekode Sinyal Audio</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const formUpload = document.getElementById('form-upload-container');
        const formRecord = document.getElementById('form-record-container');

        btnTabUpload.addEventListener('click', function() {
            // Tampilkan form upload
            formUpload.classList.remove('hidden');
            formRecord.classList.add('hidden');

            // Atur style tab
            btnTabUpload.classList.add('bg-gray-700', 'text-white');
            btnTabUpload.classList.remove('text-gray-400', 'hover:text-white');

            btnTabRecord.classList.add('text-gray-400', 'hover:text-white');
            btnTabRecord.classList.remove('bg-gray-700', 'text-white');
        });

        btnTabRecord.addEventListener('click', function() {
            // Tampilkan form rekam
            formRecord.classList.remove('hidden');
            formUpload.classList.add('hidden');

            // Atur style tab
            btnTabRecord.classList.add('bg-gray-700', 'text-white');
            btnTabRecord.classList.remove('text-gray-400', 'hover:text-white');

            btnTabUpload.classList.add('text-gray-400', 'hover:text-white');
            btnTabUpload.classList.remove('bg-gray-700', 'text-white');
        });
    });
</script>
